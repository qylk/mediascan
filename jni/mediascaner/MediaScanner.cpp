...
MediaScanner::MediaScanner():scanedFileNumber(0),scanedFolderNumber(0)
{
...
MediaScanResult MediaScanner::processDirectory(const char* path, MediaScannerClient& client)
{
    ...
    free(pathBuffer);
    MLOGV("scaned %d folders and %d files",scanedFolderNumber,scanedFileNumber);
    return result;
}
...
MediaScanResult MediaScanner::ExtractMetaData(const char* path,MediaScannerClient& client)
{
    ...
    if(!meta->hasKey(KeyTitle))
    {
        const char* start=strrchr(path, '/');
        const char* last=strrchr(path, '.');
        meta->putItem(TYPE_C_STRING,KeyTitle,start+1,last-start-1);
    }

    if(!meta->hasKey(KeyAlbum))
    {
        const char* last=strrchr(path, '/');
        const char *start;
        for(start=last-1; start>=path; start--)
        {
            if(*start=='/')
                break;
        }
        meta->putItem(TYPE_C_STRING,KeyAlbum,start+1,last-start-1);
    }

    const uint32_t id[]= {KeyTitle,KeyArtist,KeyAlbum};
    for(size_t i=0; i<3; i++)
    {
        size_t size;
        const void *data;
        if(meta->getItem(id[i],&data,&size))
        {
            uint16_t ucs2buf[size];
            size_t outlen;
            conv_utf8_ucs2((const uint8_t*)data,size,ucs2buf,&outlen);
            char pinyin[size];
            size_t letterNum;
